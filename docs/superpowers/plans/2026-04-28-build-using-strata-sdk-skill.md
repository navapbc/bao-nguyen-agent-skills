# build-using-strata-sdk Skill Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create `skills/build-using-strata-sdk/SKILL.md` — a mirror of `build-strata-application-form` that clones the `strata-sdk-rails` repo into a local subfolder and reads its docs directly instead of generating agent rules.

**Architecture:** Single new skill file under `skills/build-using-strata-sdk/`. Mirrors the 15-step flow of the existing skill but replaces the "generate rules → read rules" pattern (Steps 7 + 10a) with "clone SDK repo → read docs from clone". All other steps (TDD, migration, views, entry point) remain structurally identical.

**Tech Stack:** Markdown only — skill file consumed by Claude Code Skill tool. Python lint script validates the file against 12 rules on CI.
...
|--------|------|----------------|
| Create | `skills/build-using-strata-sdk/SKILL.md` | New skill |
| Verify (existing) | `scripts/lint_skills.py` | Enforces 12 linting rules |
| Verify (existing) | `tests/test_lint_skills.py` | Lint rule unit tests |
...
**Files:**
- Create: `skills/build-using-strata-sdk/SKILL.md`

- [ ] **Step 1: Create the directory**

```bash
mkdir skills/build-using-strata-sdk
```

- [ ] **Step 2: Write SKILL.md**

Create `skills/build-using-strata-sdk/SKILL.md` with the following **exact** content.
Note the key difference from `build-strata-application-form`:
- Step 7 clones the SDK repo instead of generating rules.
- Step 10a reads docs from the clone instead of generated rule files.
- No `--agent` flag question (Step 4 is removed; Step 4 of the original becomes irrelevant).
- Clone lives at `<RAILS_DIR>/tmp/strata-sdk` and is `.gitignore`d.
...
**2c. All subsequent commands** in this skill (Step 5 onward) **must run from inside `<RAILS_DIR>`**.
...
Before installing any gems, confirm the active Ruby matches what the project requires.
...
- A system spec for `<ENTRY_POINT>` matching the option chosen in Step 11

Run `make test` — confirm all of the above **fail** for the right reasons.

**12b. Implement minimum to pass each spec, one at a time:**

1. Generate or hand-write the controller + views per the SDK docs.
...
Both must be green before moving to the next spec.
...
```bash
wc -l skills/build-using-strata-sdk/SKILL.md
```

Expected: under 500. If over, trim — most likely candidates are duplicate table rows or overly verbose common-pitfalls sections.

- [ ] **Step 4: Verify description length stays under 250 chars**

```bash
python3 -c "
import re, sys
text = open('skills/build-using-strata-sdk/SKILL.md').read()
m = re.search(r'^description:\s*(.+)$', text, re.MULTILINE)
desc = m.group(1).strip() if m else ''
print(len(desc), repr(desc))
sys.exit(0 if len(desc) <= 250 else 1)
"
```

Expected: exits 0, prints length ≤ 250.

---

## Task 2: Run the linter and fix any violations

**Files:**
- Read: `scripts/lint_skills.py`
- Read: `tests/test_lint_skills.py`
...
Expected output: all skills pass, including `build-using-strata-sdk`.

- [ ] **Step 2: Run the linter unit tests**

```bash
python -m pytest tests/test_lint_skills.py -v
```

Expected: all existing tests still pass (no regressions from the new skill file).

- [ ] **Step 3: Fix any violations**

If lint reports errors, fix them inline in `skills/build-using-strata-sdk/SKILL.md`:

| Common violation | Fix |
|-----------------|-----|
| `name` mismatch | Ensure `name: build-using-strata-sdk` in frontmatter matches dir exactly |
| description > 250 chars | Shorten — remove "instead of generating agent rules" clause if needed |
| body > 500 lines | Trim duplicate pitfall rows; shorten step prose |
| first-person in description | Rewrite to third-person: "Adds …" not "I add …" |
| second-person in description | Rewrite: "Adds …" not "Use this to …" |

Re-run `python scripts/lint_skills.py` after each fix until clean.

- [ ] **Step 4: Commit**

```bash
git add skills/build-using-strata-sdk/SKILL.md
git commit -m "feat: add build-using-strata-sdk skill (clone-based SDK reference approach)"
```
...
- [x] **Spec coverage:** New skill mirrors all 15 steps of `build-strata-application-form`. Steps 7+10a replaced with clone+docs-read. All other steps preserved structurally. Linter validation covered in Task 2.
- [x] **Placeholder scan:** No TBD/TODO/fill-in-later present. All code blocks contain exact commands or content.
- [x] **Type consistency:** No cross-task type/method references — skill file is self-contained Markdown.
